...
class Cache:
    ...
    def __init__(self, slots=16, main_memory=None):
        ...
        self.slots = slots
        self.cache_slots = [Record(slot=i, valid=0, tag=0, data=Block()) for i in range(0, slots)]
    ...
    def read(self, address):
        ...
        cache_hit = (record.tag == tag and record.valid == 1)
        if cache_hit:
            ...
            if record.dirty:
                ...
                address_to_write = get_address_to_write(record.tag, slot, offset)
                self.write_block_to_memory(address_to_write, record.data.block)
                record.dirty = 0
    ...
    def write_block_to_memory(self, address, data):
        ...
        block_starting_address = address - offset
        block_end_address = block_starting_address + 16
        print(f"Writing block {hex(block_starting_address)} -> {hex(block_end_address - 1)} to memory...")
